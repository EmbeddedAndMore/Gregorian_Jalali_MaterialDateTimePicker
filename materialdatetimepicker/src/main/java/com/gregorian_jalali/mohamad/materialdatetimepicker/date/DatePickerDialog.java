...
import com.gregorian_jalali.mohamad.materialdatetimepicker.HapticFeedbackController;
import com.gregorian_jalali.mohamad.materialdatetimepicker.R;
import com.gregorian_jalali.mohamad.materialdatetimepicker.TypefaceHelper;
import com.gregorian_jalali.mohamad.materialdatetimepicker.Utils;
import com.gregorian_jalali.mohamad.materialdatetimepicker.utils.LanguageUtils;
import com.gregorian_jalali.mohamad.materialdatetimepicker.utils.PersianCalendar;
...
public class DatePickerDialog extends DialogFragment implements
    ...
    private static final String KEY_JALALI = "jalali";
    ...
    private static final String  KEY_PERSIAN_HIGHLIGHTED_DAYS = "persian_hilighted_days";
    ...
    private PersianCalendar[] persianHighlightedDays;
    ...
    private int pOkResid = R.string.p_mdtp_ok;
    private String mOkString;
    private int mOkColor = -1;
    private int mCancelResid = R.string.mdtp_cancel;
    private int pCancelResid = R.string.p_mdtp_cancel;
    ...
    // Changes
    static boolean mIsJalali = false;
    ...
    public static DatePickerDialog newInstancePersian(OnDateSetListener callBack, int year,
          int monthOfYear,
          int dayOfMonth,boolean isJalali){
        mIsJalali = isJalali;
        return DatePickerDialog.newInstance(callBack, year, monthOfYear, dayOfMonth);
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        if(IsJalali()){
            ...
            outState.putInt(KEY_SELECTED_DAY, mPersianCalendar.getPersianMonth());
        }else{
        ...
        outState.putBoolean(KEY_JALALI,mIsJalali);
        ...
        outState.putSerializable(KEY_PERSIAN_HIGHLIGHTED_DAYS, persianHighlightedDays);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedInstanceState != null) {
            ...
            mIsJalali = savedInstanceState.getBoolean(KEY_JALALI);
            ...
            persianHighlightedDays = (PersianCalendar[])savedInstanceState.getSerializable(KEY_PERSIAN_HIGHLIGHTED_DAYS);
        ...
        if(IsJalali())
        {
            okButton.setTypeface(TypefaceHelper.get(activity, "web_Yekan"));
            if (mOkString != null) okButton.setText(mOkString);
            else okButton.setText(pOkResid);
        }else {
        ...
        if(IsJalali()) {
            cancelButton.setTypeface(TypefaceHelper.get(activity, "web_Yekan"));
            if (mCancelString != null) cancelButton.setText(mCancelString);
            else cancelButton.setText(pCancelResid);
        }else{
    ...
    private void updateDisplay(boolean announce) {
        if(IsJalali()){
            try {
                mYearView.setTypeface(TypefaceHelper.get(getActivity(), "web_Yekan"));
        ...
        if (mVersion == Version.VERSION_1) {
            ...
            if(IsJalali()) {
                mSelectedMonthTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP,28);
                mSelectedMonthTextView.setTypeface(TypefaceHelper.get(getActivity(), "web_Yekan"));
                mSelectedMonthTextView.setText(LanguageUtils.
                        getPersianNumbers(mPersianCalendar.getPersianMonthName()));
                mSelectedDayTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP,32);
                mSelectedDayTextView.setTypeface(TypefaceHelper.get(getActivity(), "web_Yekan"));
        ...
        if (mVersion == Version.VERSION_2) {
            if(IsJalali()) {
                mSelectedDayTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP,16);
                mSelectedDayTextView.setTypeface(TypefaceHelper.get(getActivity(), "web_Yekan"));
                mSelectedDayTextView.setText(mPersianCalendar.getPersianWeekDayName() + "," + LanguageUtils.
                        getPersianNumbers(String.valueOf(mPersianCalendar.getPersianDay())) + " " + mPersianCalendar.getPersianMonthName());
            }else
    ...
    public void setJalali(boolean IsJalali){
        mIsJalali = IsJalali;
    }
    ...
    @Override
    public PersianCalendar[] persianGetHighlightedDays() {
        return persianHighlightedDays;
    }
    ...
    public void setHighlightedDays(PersianCalendar[] highlightedDays) {
        ...
        this.persianHighlightedDays = highlightedDays;
    }

    @Override
    public PersianCalendar[] persianGetSelectableDays() {
     ...
     */
    @Override
    public PersianCalendar persianGetMinDate() {
        return mMinDate;
    }
     ...
     */
    @Override
    public PersianCalendar persianGetMaxDate() {
        return mMaxDate;
    }

    @Override
    public int getPersianMinYear() {
