...
public abstract class MonthView extends View {
    ...
    protected int DEFAULT_WEEK_START = Calendar.SUNDAY;
    ...
    protected final PersianCalendar mPersianDayLabelCalendar;
    ...
    public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
        ...
        if(DatePickerDialog.IsJalali())
            DEFAULT_WEEK_START = Calendar.SATURDAY;

        mPersianDayLabelCalendar = new PersianCalendar();
    ...
    private String getMonthAndYearString() {
        if(DatePickerDialog.IsJalali()){
            ...
            String MonthAndYear = LanguageUtils.getPersianNumbers(
    ...
    protected void drawMonthTitle(Canvas canvas) {
        ...
        mMonthTitlePaint.setTypeface(TypefaceHelper.get(this.context, "web_Yekan"));
    ...
    protected void drawMonthDayLabels(Canvas canvas) {
        ...
        for (int i = 0; i < mNumDays; i++) {
            ...
            String weekString = "";
            if(DatePickerDialog.IsJalali()){
                mPersianDayLabelCalendar.set(Calendar.DAY_OF_WEEK, calendarDay);
                String localWeekDisplayName = mPersianDayLabelCalendar.getPersianWeekDayName(); // TODO: RTLize
                weekString = localWeekDisplayName.substring(0, 1);
            }else{
                mDayLabelCalendar.set(Calendar.DAY_OF_WEEK, calendarDay);
                weekString = getWeekDayLabel(mDayLabelCalendar);
            }
            mMonthDayLabelPaint.setTypeface(TypefaceHelper.get(this.context, "web_Yekan"));
    ...
    protected boolean isOutOfRange(int year, int month, int day) {
        if (mController.persianGetSelectableDays() != null) {
    ...
    private boolean isSelectable(int year, int month, int day) {
        PersianCalendar[] selectableDays = mController.persianGetSelectableDays();
    ...
    private boolean isBeforeMin(int year, int month, int day) {
        ...
        PersianCalendar minDate = mController.persianGetMinDate();
        if (minDate == null) {
            return false;
        }
    ...
    private boolean isAfterMax(int year, int month, int day) {
        ...
        PersianCalendar maxDate = mController.persianGetMaxDate();
        if (maxDate == null) {
            return false;
        }
    ...
    protected boolean isHighlighted(int year, int month, int day) {
        if(DatePickerDialog.IsJalali()){
            PersianCalendar[] highlightedDays = mController.persianGetHighlightedDays();
    ...
    protected class MonthViewTouchHelper extends ExploreByTouchHelper {
        ...
        private final PersianCalendar mPersianTempCalendar = new PersianCalendar();
        ...
        protected CharSequence getItemDescription(int day) {
            ...
            if(DatePickerDialog.IsJalali()){
                mPersianTempCalendar.setPersianDate(mYear, mMonth, day);
                date = LanguageUtils.getPersianNumbers(mPersianTempCalendar.getPersianLongDate());
            }else{
                mTempCalendar.set(mYear, mMonth, day);
                date = DateFormat.format(DATE_FORMAT,
