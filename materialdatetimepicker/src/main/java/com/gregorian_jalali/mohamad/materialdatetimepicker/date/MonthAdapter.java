...
import com.gregorian_jalali.mohamad.materialdatetimepicker.date.MonthAdapter.MonthViewHolder;
...
public abstract class MonthAdapter extends RecyclerView.Adapter<MonthViewHolder> implements MonthView.OnDayClickListener {
    ...
    @Override public int getItemCount() {
        if(DatePickerDialog.IsJalali()){
            return ((mController.getPersianMaxYear() - mController.getPersianMinYear()) + 1) * MONTHS_IN_YEAR;
        }else{
    ...
    static class MonthViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(int position, DatePickerController mController, CalendarDay selectedCalendarDay) {
            int month = (position + mController.getStartDate().get(Calendar.MONTH)) % MONTHS_IN_YEAR;
            int year = (position + mController.getStartDate().get(Calendar.MONTH)) / MONTHS_IN_YEAR + mController.getMinYear();

            int selectedDay = -1;
            if(DatePickerDialog.IsJalali()){

                month = position % MONTHS_IN_YEAR;
                year = position / MONTHS_IN_YEAR + mController.getPersianMinYear();
